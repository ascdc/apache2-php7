...
RUN DEBIAN_FRONTEND=noninteractive && apt-get update && \
	...
	echo "extension=uploadprogress.so" | sudo tee /etc/php/7.0/mods-available/uploadprogress.ini && \
	echo "extension=memcache.so" | sudo tee /etc/php/7.0/mods-available/memcache.ini && \
	ln -sf /etc/php/7.0/mods-available/imagick.ini /etc/php/7.0/apache2/conf.d/20-imagick.ini && \
	ln -sf /etc/php/7.0/mods-available/uploadprogress.ini /etc/php/7.0/apache2/conf.d/20-uploadprogress.ini && \
	ln -sf /etc/php/7.0/mods-available/memcache.ini /etc/php/7.0/apache2/conf.d/20-memcache.ini
